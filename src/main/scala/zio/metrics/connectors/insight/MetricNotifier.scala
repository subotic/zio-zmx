...
case class MetricsUpdate(
  ...
  subId: String,
  when: Instant,
  states: Map[MetricKey[MetricKeyType], MetricState])
...
object MetricNotifier {
  ...
  def live: ZLayer[Clock with Random, Nothing, MetricNotifier] =
    ZLayer {
      for {
        rnd   <- ZIO.service[Random]
        clk   <- ZIO.service[Clock]
        state <- Ref.Synchronized.make(NotifierState.empty)
      } yield new MetricNotifierImpl(rnd, clk, state) {}
    }
  ...
  private[MetricNotifier] case class ConnectedClient(
    ...
    keys: Hub[Seq[MetricKey.Untyped]],
    clk: Clock,
